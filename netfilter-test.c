...
int bad_or_not (uint8_t * http, uint64_t http_len, const char * bad_host){
	...
	for(i = 0 ; i<=http_len - 6; i++){
		...
		uint8_t *candidate = http +i +6;
		uint64_t candidate_len = 0;

		for(j = 0; j +i +6 <http_len; j++){
			if(candidate[j] =='\n' || candidate[j] =='\r'){
				break;
			}
		}
		...
		if(candidate_len == strlen(bad_host) && memcmp(candidate,bad_host,strlen(bad_host))==0){
			return 1;
		}
	}
	return 0;
...
int main(int argc, char **argv)
{
	...
	qh = nfq_create_queue(h, queue, &cb, (void *)bad_host);
	if (!qh) {
